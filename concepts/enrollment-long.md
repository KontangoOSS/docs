...
## What the cryptography is doing

If you want the technical detail: enrollment is a one-time-token (OTT) flow built on the OpenZiti SDK.

1. The installer requests a one-time enrollment token from the controller. The token is short-lived (180 minutes by default) and can only be used once.
2. The device generates an ECDSA keypair locally. **The private key never leaves the device.**
3. The device sends a Certificate Signing Request to the controller along with the OTT.
4. The controller validates the OTT, validates the CSR, and signs a client certificate scoped to the device's identity. The signing CA is your platform's intermediate CA, ultimately rooted in the trust anchor in your secrets vault.
5. The signed cert plus the controller's CA bundle are returned to the device. The agent stores them, then dials the controller's edge to start the tunnel.

The cert has a Subject Alternative Name that includes the SPIFFE URI for the device (`spiffe://kontango/device/<id>`), the device's hostname, and (optionally) any custom ID provided. Validity is one year by default, with auto-renewal kicking in at 75% of lifetime.

You don't need to know any of this to enroll. You do need to know it if you're operating the platform and something goes wrong.
...
On the platform side, enrollment events flow through a small set of services:

- The **enrollment controller** (the API the installer hits) accepts the request, runs the decision engine, and either issues an OTT or queues for human review
- The **OpenZiti controller** signs the certificate when the OTT is presented
- The **secrets vault** holds the intermediate CA private key — it's used to sign certs but never leaves the vault
- The **honeypot service** logs anything that looks like an enrollment probe but isn't a real device, so you have visibility into scanning attempts

If you operate the platform, you'll see enrollments in the admin dashboard. Each one shows the device fingerprint, the score, the decision, and (if approved) the issued identity. Manual-review queue items show up as a feed; clicking approve issues the cert.
...
- **You can revoke it.** If your laptop is lost, an admin can revoke the identity and the certificate stops working immediately. Revocation is enforced server-side on every connection, so a stolen device loses access the moment the admin clicks revoke.
