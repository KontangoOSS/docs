...
## Commercial paths in detail

If you're considering paid engagement, the three real options today:

**Hands-on deployment** — fixed-scope project to stand up Kontango on your hardware. We work with your IT team (or you) to inventory the hardware, design the cluster topology, install the platform, configure the services you need, and hand off. Typical timeline: 2–6 weeks depending on scope. Includes documentation specific to your deployment.

**Operate it for you** — managed Kontango. You bring the hardware (or use cloud VMs), we run the platform on it. Monthly engagement, includes monitoring, patching, on-call response, capacity planning. Useful when you want the data sovereignty without the operational burden.

**Custom services** — building specific functionality on top of Kontango. Examples: custom enrollment integrations, business-specific services, compliance reporting, audit pipelines. Hourly or fixed-bid depending on scope.

For any of these, start at `hello@kontango.io` and we'll figure out which fit.
...
If you can't run any commands because the network or device is broken, that's fine — just say so. The admin can usually figure things out from the symptom and the hostname alone.

## Severity guidelines

These help us prioritize:

- **URGENT** — security incident, data exposure, full outage affecting your customers, you're locked out before a critical event
- **High** — service down for >10 min, enrollment broken on multiple devices, blocked from critical workflow
- **Normal** — single user can't reach a service, a feature inside a service is misbehaving, questions about config
- **Low** — feature requests, doc clarifications, "this would be nice"

Put one of those words in the subject line. We respond to URGENT within an hour during business hours, and within a few hours otherwise. Normal severity ticket response is typically same-day.
...
If you find a bug, fix something, or want to improve the docs, the docs repo accepts pull requests. Same goes for the platform code. Open source includes you.

## Where to next

- [When things break](when-things-break) — try this first
- [The eight concepts](.) — useful background if you're new
