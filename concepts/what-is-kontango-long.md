...
A Kontango deployment has four layers, and they're worth knowing if you're going to operate one:

1. **The controllers** — typically three nodes running in a Raft cluster. They hold the source of truth: who's enrolled, what services exist, which roles are allowed where. If one controller dies, the other two keep the network running.
2. **The overlay network** — built on OpenZiti. This is the part that gives you `.tango` addresses, encrypted tunnels, and identity-based routing. Every connection is mutually authenticated.
3. **Edge proxies** — Caddy on each controller handles TLS termination, public-facing services (when you want them), and the auth gateway for OIDC. Internal services go straight through the overlay; public ones land here first.
4. **Service hosts** — your actual workloads. Forgejo for git, OpenBao for secrets, Zitadel for identity, Grafana for dashboards, Ticketarr for issue tracking, anything else you want to run. Each has an identity, each has policies, each is reachable at a `.tango` address from any enrolled device.

This separation matters because you can grow into Kontango. Start with one node running everything; add more nodes when you outgrow it; replace any layer with a different open-source tool if you don't like the default choice. Nothing is locked in.
...
The whole platform is open source. The code is in the open, the design is in the open, and you can run it without paying anyone. That's the same model RedHat made popular: the software is free, the help is the product.

If you want to set Kontango up yourself, read the docs and go. If you want someone to deploy it, run it for you, or build something specific on top of it, that's available too. We're not selling licenses. We're not gating features. The way you choose to use this is up to you.
...
Three commercial paths are real today:

- **Hands-on deployment** — paid help to stand up Kontango on your hardware, integrate with your existing systems, and hand it off
- **Operate it for you** — managed Kontango, where you bring the hardware and we run the platform on it
- **Custom services** — building a specific service or integration on top of Kontango that fits your business

None of these gate you out of the open-source path. The same code, the same docs, the same community.
